...
class CrimeMining:
    ...
    def main(self, content,Fileflg):
        ...
        #对事件网络进行图谱化展示
        self.graph_shower.create_page(events,Fileflg)



#————————————————————————————————启动方式——————————————————————————————————————————————————————————————
# def TextrankAndDraw(content):
#     handler=CrimeMining()
#     handler.main(content)
#     os.system('firefox /home/yuanzhu/Desktop/Crawler/CrawlerAndAnalyse/TextGraph/graph_show.html')
#————————————————————————————————————————————————————————————————————————
...
def localTest():
    ...
    for file in FileList:
        ...
        dict = GetDictFromJsonFile(path)
        content=dict['content']
        FileFlg = file.replace('.json','')
        handler.main(content,FileFlg)
        print('filename  = ',file)
        filepath = os.path.join(os.getcwd(), '{}.html'.format(FileFlg))
        os.system('mv  {} /home/yuanzhu/测试文件夹/美食/'.format(filepath))
        CurrentPath=os.path.join('/home/yuanzhu/测试文件夹/美食/',FileFlg)
        os.system('firefox {}'.format(CurrentPath))
