...
class GraphShow():
    ...
    def create_page(self, events,Fileflg):
        nodes = []
        for event in events:
            nodes.append(event[0])
            nodes.append(event[1])
        ...
        self.create_html(data_nodes, data_edges,Fileflg)
        return

    # '''生成html文件'''
    # def create_html(self, data_nodes, data_edges):
    #     savepath =os.path.join(os.getcwd(),'graph_show.html')
    #     f = open(savepath, 'w+')
    ...
    def create_html(self, data_nodes, data_edges,Fileflg):
        savepath = os.path.join(os.getcwd(), '{}.html'.format(Fileflg))
        f = open(savepath, 'w+')
        html = self.base.replace('data_nodes', str(data_nodes)).replace('data_edges', str(data_edges))
        f.write(html)
        f.close()
